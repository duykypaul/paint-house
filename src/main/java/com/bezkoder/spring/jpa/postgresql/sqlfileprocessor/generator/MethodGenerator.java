...
import com.bezkoder.spring.jpa.postgresql.sqlfileprocessor.QueryBlockCreator;
import com.bezkoder.spring.jpa.postgresql.sqlfileprocessor.SqlFromResource;
import com.bezkoder.spring.jpa.postgresql.sqlfileprocessor.TypeRecognizer;
                ...
                methodAnnotations
                        ...
                        .map(annotation -> {
                            AnnotationSpec.Builder annotationSpec = AnnotationSpec
                                    .builder(ClassName.bestGuess(annotation.getAnnotationType().toString()));

                            annotation
                                    .getElementValues()
                                    .keySet()
                                    .forEach(key -> annotationSpec.addMember(key.toString().replaceAll("\\(\\)", ""),
                                            annotation.getElementValues().get(key).toString()));
