...
import com.bezkoder.spring.jpa.postgresql.sqlfileprocessor.generator.MethodGenerator;
import com.bezkoder.spring.jpa.postgresql.sqlfileprocessor.generator.TypeGenerator;
    ...
    public boolean process(Set<? extends TypeElement> annotations, RoundEnvironment roundEnv) {
        ...
        for (Element annotatedElement : elements) {
            ...
            if (annotatedElement.getKind().isInterface() && !annotatedElement.getSimpleName().toString().endsWith(getClassPostfix())) {
                ...
                System.out.println("full repository name = " + annotatedElement);
                ...
                List<MethodSpec> methods = annotatedElement
                        ...
                        .map(method -> {
                            ...
                            Annotation annotation = method.getAnnotation(SqlFromResource.class);
                            ...
                            List<String> paramNames = m.isEmpty() ? new ArrayList<>() :
                                    m.stream().map(p -> p.getSimpleName().toString()).collect(Collectors.toList());
                            ...
                            List<String> paramTypes = types.isEmpty() ? new ArrayList<>() :
                                    types.stream().map(TypeMirror::toString).collect(Collectors.toList());
                ...
                System.out.println("source code = " + annotatedElement);
            }
        }
